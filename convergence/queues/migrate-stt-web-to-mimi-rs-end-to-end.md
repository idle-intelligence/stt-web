STATUS: COMPLETE — 2026-05-03, self-review at convergence/queues/migrate-stt-web-to-mimi-rs-end-to-end-self-review.md
...
- [x] **Discovery: Make model files available for e2e tests** (human action required). Tasks 5 and 5b require model files that are not in the repo. Needed at `stt-web/models/`: `stt-1b-en_fr-q4.gguf`, `mimi.safetensors`, `tokenizer.model`; and at `stt-web/web/`: `test-loona.wav`, `test-bria.wav`. Without these, `cargo test` exits 0 but 14/15 tests silently no-op. The loop cannot download from HuggingFace (prohibited by hard rules). Owner must place files and mark this task [x] to unblock Tasks 5, 5b, and the Acceptance check.

- [x] **5. Run stt-web tests** (single-shot, ~15m). Run `cargo test --features "wgpu" -- --test-threads=1` from stt-web repo root. All tests must pass.
